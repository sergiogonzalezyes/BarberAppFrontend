...
export default {
  ...
  data() {
    return {
      ...
      paymentMethods: [], // Array to store available payment methods
  ...
  methods: {
    async fetchBarbersAndTimeSlots(serviceId) {
      try {
        const response = await axios.get(
          `http://localhost:5001/service/${serviceId}/availability`
        );
    ...
    async fetchAvailableDatesForBarber(barber_id) {
      try {
        const response = await axios.get(
          `http://localhost:5001/schedule/${barber_id}/available-dates`
        );
        const data = response.data;

        this.availableDates = data.available_dates.map((dateString) => {
    ...
    async fetchAvailableTimeSlotsForDate() {
      try {
        const response = await axios.get(
          `http://localhost:5001/schedule/${this.selectedBarber}/available-time-slots?date=${this.selectedDateFormatted}`
        );
        const data = response.data;
        console.log("Available time slots:", data);

        // Convert the object to an array of time slot objects and format the time
        this.availableTimeSlots = Object.keys(data.available_time_slots).map(
          (slotNumber) => {
    ...
    submitForm() {
      if (this.$refs.form.validate()) {
        ...
        const selectedTimeSlot = this.availableTimeSlots.find(
          (slot) => slot.slotNumber === selectedTimeSlotId
        );
    ...
    async fetchPaymentMethods() {
      try {
        ...
        // Assuming the response is an array of payment methods with 'id' and 'name' properties
